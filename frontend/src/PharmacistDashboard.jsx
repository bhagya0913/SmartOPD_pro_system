...
import {
    ...
    ExternalLink, ChevronDown, Barcode, WifiOff, Wifi, ClipboardCheck,
    Ban, Info, HelpCircle
} from 'lucide-react';
...
function downloadReportHTML(reportTitle, dateFrom, dateTo, bodyHTML) {
    ...
    const fullHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${reportTitle}</title>${style}</head><body>
        <div class="header">
...
function openReportTab(reportTitle, dateFrom, dateTo, bodyHTML) {
    ...
    const style = `<style>
        *{box-sizing:border-box}body{font-family:'Segoe UI',Arial,sans-serif;color:#0f172a;margin:0;padding:0 24px;font-size:13px;background:#fff}
        ...
        table{width:100%;border-collapse:collapse;font-size:12px;margin-bottom:16px}
        thead{background:#1a6b3c}th{padding:8px 12px;text-align:left;font-weight:700;font-size:10px;text-transform:uppercase;letter-spacing:.06em;color:white}
        td{padding:7px 12px;border-bottom:1px solid #f1f5f9}tr:nth-child(even) td{background:#fafafa}
        .no-data{text-align:center;color:#94a3b8;font-style:italic;padding:20px}
        .footer{margin-top:30px;padding-top:10px;border-top:2px solid #e2e8f0;font-size:10px;color:#94a3b8;display:flex;justify-content:space-between}
        .green{color:#16a34a}.amber{color:#d97706}.red{color:#dc2626}.blue{color:#2563eb}
    </style>`;
    const fullHtml = `<!DOCTYPE html><html><head><meta charset="UTF-8"><title>${reportTitle}</title>${style}</head><body>
        ...
        ${bodyHTML}
        <div class="footer">
            <span>SmartOPD — Pharmacy Unit | Internal use only</span>
            <span style="color:#dc2626;font-weight:700">CONFIDENTIAL</span>
        </div>
    </body></html>`;
        ...
        <div className="ph-shell">
            ...
            <div className="ph-main">
                ...
                <div className="ph-content">
                    ...
                    {activeTab === 'dispense'      && <DispenseMeds user={user}/>}
        ...
        <div className="ph-section">
            ...
            <div className="ph-card">
                <div className="ph-card-head">
                    <h3><Pill size={14}/> Pending Dispensations</h3>
                </div>
                {loading ? <Spinner/>
                : queue.length === 0 ? <Empty icon={Pill} text="No pending prescriptions."/>
                : (
                        ...
                        <tbody>
                            {queue.map((r, i) => (
                                <tr key={r.record_id || i}>
                                    ...
                                    <td className="ph-mono ph-dimmed" style={{fontSize:'.72rem'}}>{r.barcode || '—'}</td>
                                    ...
                                    <td><span className="ph-badge ph-badge-amber">Pending</span></td>
                                </tr>
                            ))}
                        </tbody>
                    </table>
...
// ══════════════════════════════════════════════════════════════════════════════
//  PRESCRIPTION PANEL — professional Rx UI with per-medication dispensing
// ══════════════════════════════════════════════════════════════════════════════
function PrescriptionPanel({ pid, patient, prescriptions, pharmacistId, onUpdate }) {
    const [fulfilling,   setFulfilling]   = useState(null);
    // medStatus: { [record_id]: { [medIndex]: 'dispensed'|'unavailable'|null } }
    const [medStatus,    setMedStatus]    = useState({});
    // pharmacistNotes: { [record_id]: string }
    const [pharmacistNotes, setPharmacistNotes] = useState({});
    const [savingNote,   setSavingNote]   = useState(null);

    const pendingCount = prescriptions.filter(rx => !rx.fulfilled).length;

    // Parse prescription_details into structured medication list
    const parseMeds = (raw) => {
        if (!raw) return [];
        const lines = raw.split('\n').filter(l => l.trim());
        return lines.map((line, i) => {
            // Try to detect structured lines like "1. MedName | Dosage | Duration"
        ...
        <div className="ph-patient-panel">
            ...
            <div className="ph-pt-header">
                ...
                <div className="ph-pt-info">
                    ...
                    <div className="ph-pt-meta">
                        ...
                        <span className="ph-pt-barcode"><Barcode size={12}/> {patient.barcode}</span>
                    </div>
                </div>
            ...
            {prescriptions.length === 0
                ...
                : prescriptions.map((rx, i) => {
                    ...
                    const hasUnavailable = meds.some((_,idx) => rxMedStatus[idx] === 'unavailable');
                    ...
                    return (
                        <div key={rx.record_id || i} className={`ph-rx-card ${rx.fulfilled ? 'ph-rx-fulfilled' : ''}`}>
                                ...
                                <div className="ph-rx-hdr-left">
                                    ...
                                    <div className="ph-rx-doc-info">
                                        <span className="ph-rx-doctor">
                                            ...
                                            {rx.doctor_staff_id && (
                                                <span className="ph-staff-id-tag" style={{marginLeft:'8px'}}>
                                ...
                                <div className="ph-rx-hdr-right">
                                    {rx.fulfilled
                                        ? <div className="ph-rx-dispensed-badge">
                                            ...
                                            <div>
                                                <div>Dispensed</div>
                                                <div style={{fontSize:'.68rem',fontWeight:500,opacity:.8}}>{fmtDate(rx.fulfilled_at)}</div>
                                            </div>
                                          </div>
                            ...
                            {/* Rx body — professional prescription layout */}
                                    ...
                                    <table className="ph-rx-med-table">
                                        <thead>
                                            <tr>
                                                ...
                                                <th>Dosage</th>
                                                <th>Duration</th>
                                                {!rx.fulfilled && <th style={{width:'160px'}}>Pharmacist Action</th>}
                                                {!rx.fulfilled && <th style={{width:'90px'}}>Status</th>}
                                            </tr>
                                        </thead>
                                        <tbody>
                                            {meds.map((med, idx) => {
                                                    ...
                                                    <tr key={idx} className={
                                                        ...
                                                        <td>
                                                            <span className="ph-med-name"
                                                                style={st === 'dispensed' ? {textDecoration:'line-through',opacity:.6} : {}}>
                                                                {med.name}
                                                            </span>
                                                        </td>
                                                        <td className="ph-mono" style={{fontSize:'.78rem',color:'#64748b'}}>
                                                            {med.dosage || '—'}
                                                        </td>
                                                        <td className="ph-mono" style={{fontSize:'.78rem',color:'#64748b'}}>
                                                            {med.duration || '—'}
                                                        ...
                                                        {!rx.fulfilled && (
                                                            <td>
                                                                ...
                                                                {!st                  && <span style={{color:'#94a3b8',fontSize:'.72rem'}}>—</span>}
                                                            </td>
                                                        )}
                                                    </tr>
                                                );
                                ...
                                <div className="ph-rx-notes-area">
                                    ...
                                    <div style={{display:'flex',gap:'8px',alignItems:'flex-start'}}>
                                        <textarea
                                            ...
                                            rows={2}
                                            style={{minHeight:'60px',fontSize:'.82rem'}}
                                            placeholder="Add any notes about this prescription, e.g. substitution, counselling given, unavailable items…"
                                        ...
                                        <button className="ph-btn-ghost ph-btn-sm"
                                            ...
                                            {savingNote === rx.record_id ? <><div className="ph-btn-spin"/>…</> : <><Save size={12}/> Save</>}
                                        </button>
                                    </div>
                                </div>
                            )}

                            {/* Rx footer — overall dispense action */}
                            {!rx.fulfilled && (
                                <div className="ph-rx-footer">
                                    ...
                                    <div className="ph-rx-footer-actions">
                                        ...
                                        {allMarked && !rx.fulfilled && (
                                            <span className="ph-rx-all-marked">
                                                <Check size={12}/> All items marked
                                            </span>
                                        )}
                                    </div>
                                </div>
                            ...
                            {/* Already fulfilled note */}
                            {rx.fulfilled && (
                                <div className="ph-rx-fulfilled-bar">
                                    <CheckCircle2 size={14}/> This prescription was dispensed on {fmtDTime(rx.fulfilled_at)}
                                </div>
                            )}
                        </div>
                    );
...
// ══════════════════════════════════════════════════════════════════════════════
//  DISPENSE MEDICATIONS  — barcode + NIC only, with hardware scanner support
// ══════════════════════════════════════════════════════════════════════════════
function DispenseMeds({ user }) {
    ...
    // openPatients: [{ pid, patient, prescriptions }]
    ...
    // Auto-focus input whenever mode changes
    useEffect(() => { inputRef.current?.focus(); }, [mode]);

    // ── Hardware barcode scanner listener ──────────────────────────────────────
    // Barcode scanners typically send characters very fast then press Enter.
    // We detect this by timing: if > 4 chars arrive within 100ms, treat as scan.
    useEffect(() => {
        const onKeyDown = (e) => {
            // Only intercept when dispense tab is active and input isn't focused
            ...
            if (e.key.length === 1) { // printable char
                ...
                scanTimer.current = setTimeout(() => {
                    // If buffer has chars but no Enter came in time, clear it (manual typing, not scanner)
    ...
    }, [openPatients]); // re-attach when openPatients changes so triggerSearch has fresh closure
    ...
    const handlePrescriptionsUpdate = (pid, freshPrescriptions) => {
    ...
    return (
        <div className="ph-section">
            <div className="ph-section-head">
                ...
                <button className="ph-btn-ghost ph-btn-sm"
                    onClick={() => setScannerInfo(s => !s)}>
                    <Barcode size={14}/> Scanner Help
                </button>
            </div>

            {/* ── Barcode scanner info panel ────────────────────────────── */}
            {scannerInfo && (
                <div className="ph-scanner-info-card">
                    ...
                    <div className="ph-sic-body">
                        <div className="ph-sic-row">
                            <div className="ph-sic-step">
                                <div className="ph-sic-num">1</div>
                                <div>
                                    <strong>Connect your scanner</strong>
                                    <p>Plug in your USB barcode scanner. It works as a keyboard — no driver needed.</p>
                                </div>
                            </div>
                            <div className="ph-sic-step">
                                <div className="ph-sic-num">2</div>
                                <div>
                                    <strong>Scan anywhere on this page</strong>
                                    <p>You do <em>not</em> need to click the input box. Simply scan the barcode and the system will automatically detect it and search the patient.</p>
                                </div>
                            </div>
                            <div className="ph-sic-step">
                                <div className="ph-sic-num">3</div>
                                <div>
                                    <strong>Or click the input and scan</strong>
                                    <p>You can also click the search box first, then scan — both methods work.</p>
                                </div>
                            </div>
                        </div>
                        <div className="ph-sic-tip">
                            <Info size={13}/>
                            <span>Most USB scanners automatically press <kbd>Enter</kbd> after scanning. The system detects scanner input vs. manual typing by the speed of key input.</span>
                        </div>
                    </div>
                </div>
            )}
            ...
            <div className="ph-lookup-card">
                {/* Mode tabs — barcode and NIC only */}
        ...
        switch(rptId) {
            case 'daily_received': {
                ...
                return `
                    ...
                    <table><thead><tr><th>Date</th><th>Day</th><th>Prescriptions Received</th></tr></thead><tbody>
            ...
            case 'dispensed': {
                ...
                return `
                    ...
                    <table><thead><tr><th>#</th><th>Patient</th><th>NIC</th><th>Doctor</th><th>Prescribed On</th><th>Dispensed On</th></tr></thead><tbody>
            ...
            case 'pending': {
                ...
                return `
                    ...
                    <table><thead><tr><th>#</th><th>Patient</th><th>NIC</th><th>Doctor</th><th>Prescribed On</th><th>Days Outstanding</th></tr></thead><tbody>
                    ${rows.length ? rows.map((r,i)=>{
                        const days = Math.floor((new Date()-new Date(r.consultation_day))/(1000*60*60*24));
                        const urgClass = days>7?'class="red"':days>3?'class="amber"':'class="green"';
                        return `<tr><td>${i+1}</td><td><strong>${r.patient_name||'—'}</strong></td><td>${r.nic||'—'}</td><td>Dr. ${r.doctor_name||'—'}</td><td>${fmtDate(r.consultation_day)}</td><td ${urgClass}>${days} day${days!==1?'s':''}</td></tr>`;
                    }).join('') : noData()}
                    </tbody></table>`;
            }
            case 'most_prescribed': {
                ...
                return `
                    ...
                    <table><thead><tr><th>Rank</th><th>Medication</th><th>Times Prescribed</th><th>Percentage</th></tr></thead><tbody>
            ...
            case 'freq_by_doctor': {
                ...
                return `
                    <div class="stat-grid">${sc(rows.length,'Prescribing Doctors')}${sc(rows.reduce((s,r)=>s+(r.count||0),0),'Total Prescriptions','#2563eb')}</div>
                    <div class="section-title">Prescription Frequency by Doctor</div>
                    <table><thead><tr><th>Rank</th><th>Doctor</th><th>Staff ID</th><th>Prescriptions Issued</th><th>% of Total</th></tr></thead><tbody>
                    ${rows.length ? rows.sort((a,b)=>(b.count||0)-(a.count||0)).map((r,i)=>{
                        const tot = rows.reduce((s,x)=>s+(x.count||0),0);
                ...
                <div className="ph-report-step">
                    ...
                    <div className="ph-card">
                        <div style={{padding:'16px 18px'}}>
                            ...
                            {dateMode === 'single' ? (
                                <div className="ph-fg" style={{maxWidth:'220px',marginTop:'14px'}}>
                            ...
                            <button className="ph-btn-primary" style={{width:'100%',justifyContent:'center',marginTop:'14px',padding:'11px 20px'}}
...
// ══════════════════════════════════════════════════════════════════════════════
//  FEEDBACK — col mismatch fixed: uses date_submitted (DB column name)
// ══════════════════════════════════════════════════════════════════════════════
function PharmacistFeedback({ user }) {
    ...
    return (
        <div className="ph-section">
            ...
            <div className="ph-card" style={{ maxWidth: 600, marginBottom: '20px' }}>
                <div className="ph-card-head"><h3><MessageSquare size={14}/> Submit Feedback</h3></div>
                <form onSubmit={handleSubmit} style={{ padding: '16px' }}>
                    <div className="ph-fg" style={{ marginBottom: '14px' }}>
                    ...
                    <button type="submit" className="ph-btn-primary"
                        ...
                        {saving
                            ? <><div className="ph-btn-spin"/> Submitting…</>
                            : <><Send size={14}/> Submit Feedback</>}
                    </button>
                </form>
            </div>

            {/* History — fixed col mismatch: date_submitted is the correct DB column */}
                        ...
                        {history.map((f, i) => (
                            <div key={f.feedback_id || i} className="ph-feedback-item">
                                <div className="ph-feedback-meta">
                                    {/* FIX: Try both column names — date_submitted is the DB col */}
